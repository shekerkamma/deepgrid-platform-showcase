...
export interface GraphRAGResult {
  ...
  citation: {
    ...
    pdfSize: string;
    specPath: string;
    /** showcase: the place on this site that shows the source (a slide, dossier or memorandum chapter) */
    nav?: string;
  };
  technicalDetails: {
    summary: string;
...
const SHOWCASE_RELATED = [
  ...
  { label: 'Revenue Ramp to FY2032', query: 'How does revenue grow to FY2032?' },
  { label: 'Fifteen Products, One Chip', query: 'How do the fifteen products fit together?' },
  { label: 'Investment Risks', query: 'What are the investment risks?' }
];
...
export function executeGraphRAG(rawQuery: string, sem?: SemanticScores | null): GraphRAGResult {
  ...
  if (matchedTheme && maxThemeScore >= 15) {
    ...
    return {
      ...
      technicalDetails: isShowcaseDoc(matchedTheme.docNum) ? {
        summary: `Sources for ${matchedTheme.title}:`,
        specPoints: matchedTheme.refLinks.map(l => `${l.label}: ${l.description}`)
          .concat([`Primary source: ${matchedTheme.docTitle} · ${matchedTheme.section}${matchedTheme.page ? ` · ${matchedTheme.page}` : ''}`]),
        deepLink: {
          label: matchedTheme.refLinks[0]?.label || 'Open the source',
          hash: matchedTheme.refLinks[0]?.hash || matchedTheme.nav || 'investment',
          context: 'Figures are management projections, checked against the cited source.'
        }
      } : {
      ...
      relatedTopics: isShowcaseDoc(matchedTheme.docNum) ? SHOWCASE_RELATED.filter(t => t.query.toLowerCase() !== q) : [
  ...
  // Showcase fallback: this site's content has no silicon boilerplate; answer from the item and the source
  const showcaseItem = matchedItem && matchedItem.id === primarySeed.id && matchedItem.id.startsWith('sc-') ? matchedItem : null;
  if (showcaseItem || (!(matchedItem && matchedItem.id === primarySeed.id) && isShowcaseDoc(bestChunk.docNum))) {
    const topChunks = scoredChunks.slice(0, 4).map(s => s.chunk);
    const pageLinks = topChunks.filter(c => c.nav).slice(0, 3).map(c => ({
      label: `Open: ${c.section.slice(0, 40)}`, hash: c.nav!, description: `${c.docTitle}${c.pageLabel ? ` · ${c.pageLabel}` : ''}`
    }));
    ...
    return {
      ...
      matchedItem,
      matchedDoc: primaryDoc,
      answer: showcaseItem ? showcaseItem.summary : cleanExtractedText(bestChunk.text).slice(0, 360) + '…',
      explanation: showcaseItem ? [`${showcaseItem.tagline}.`, ...topChunks.slice(0, 2).map(quote)] : topChunks.slice(0, 3).map(quote),
      keyBusinessFacts: showcaseItem ? showcaseItem.keyFacts.slice(0, 5) : topChunks.slice(0, 4).map(c => `${c.docTitle} · ${c.section}${c.pageLabel ? ` · ${c.pageLabel}` : ''}`),
      ...
      technicalDetails: {
        ...
        specPoints: topChunks.map(c => `${c.docTitle} · ${c.section}${c.pageLabel ? ` · ${c.pageLabel}` : ''}`),
        deepLink: { label: 'Open the primary source', hash: bestChunk.nav || showcaseItem?.actions?.[0]?.target || 'investment', context: 'Figures are management projections.' }
      },
      relatedTopics: SHOWCASE_RELATED.filter(t => t.query.toLowerCase() !== q)
    };
  }
