...
export function useNavigation(){
 ...
 const go=(hash:string,replace=false)=>{const target=hash.replace(/^#/,'');positions.current[current.current]=scrollY;const previous=parse(current.current),next=parse(target);if(next.view===previous.view&&next.params.has('product')&&!positions.current[target])positions.current[target]=scrollY;saved.current[previous.view]=current.current;history[replace?'replaceState':'pushState']({deepgrid:true},'', '#'+target);apply(target);};
