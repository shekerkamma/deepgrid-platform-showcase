// The showcase's published content as citable units: every product, every slide, every use case, every
// passage of the investment memorandum, the primary sources behind them and the two financial workbooks. Both halves of Ask DeepGrid read it from here:
...
// The primary sources behind the page, from the earlier briefing's knowledge index
// (../original-platform/src/a2ui/generated/knowledge-index.json, built by that project's
// scripts/build-knowledge-index.mjs): the 33-page Information Memorandum (OCR), the market research and
// document audits, and the competitor dossiers. Its already-cleaned passages and generated questions are
// reused as-is. Kinds that are twins of what the page already carries are left out, because that project
// measured twins costing retrieval (478 of 816 passages shared a title; docs/index-coverage.md):
//   Investor memorandum = app/report-content.ts · Slide, Video narration = the 104-slide deck · Use case = use-cases.json
const TWINS = new Set(['Investor memorandum', 'Slide', 'Video narration', 'Use case']);
export const SOURCE_INDEX = path.join(ROOT, '../original-platform/src/a2ui/generated/knowledge-index.json');
function sourceDocuments() {
  const {units} = JSON.parse(fs.readFileSync(SOURCE_INDEX, 'utf8'));
  return units.filter(u => !TWINS.has(u.kind) && (u.body || '').length > 60).map(u => ({
    id: `doc:${u.id}`, kind: 'document', title: u.title, text: u.body.replace(/\s+/g, ' ').trim(),
    // questions the passage answers (generated by that project): searched, never shown
    keys: (u.questions || []).slice(0, 4),
    noise: u.noise || 0,
    source: {kind: 'document', label: String(u.source), docKind: u.kind},
  }));
}

// The financial model and business plan workbooks in ../documents/, sheet by sheet, extracted by
// scripts/extract-workbooks.py (npm run graph:workbooks) into knowledge/workbooks.json.
